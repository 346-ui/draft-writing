...
with st.expander("ℹ️ 사용법 및 주의사항 (클릭해서 열기/닫기)", expanded=False):
    ...
    **💾 자동 저장 기능 안내**
    - 작성된 글은 서버가 아닌 브라우저(로컬 스토리지)에 저장되므로, 다른 기기나 브라우저와는 연동되지 않습니다. (예: 크롬 창에서 쓴 내용은 엣지에서 볼 수 없습니다)
    - 데이터를 직접 삭제하지 않는 한 내용은 **반영구적으로 보존**됩니다. 언제든 들어와 이어 써주세요. (삭제 예시: 시크릿 창 종료, 쿠키 삭제, 클리너 앱 등)
