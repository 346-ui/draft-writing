...
with st.expander("ℹ️ 사용법 및 주의사항 (클릭해서 열기/닫기)", expanded=False):
    ...
    **'멈추지 말고 계속 쓰세요!'**
    ...
    1.  입력창에 문장을 입력하고 **Enter**를 누르세요.
    2.  입력한 문장은 저장되며 수정할 수 없습니다.
    3.  목표 글자 수를 달성하면 글이 '언락(Unlock)' 되어 수정이 가능해집니다.
    4.  왼쪽 사이드바에서 목표 글자 수 & 몰입모드 설정이 가능합니다.

    **💾 자동 저장 기능 안내**
    - 작성된 글은 서버가 아닌 브라우저(로컬 스토리지)에 저장되므로, **다른 기기나 브라우저와는 연동되지 않습니다.**
    - 데이터를 직접 삭제하지 않는 한 내용은 **반영구적으로 보존**됩니다. (삭제 예시: 시크릿 창 종료, 쿠키 삭제, 클리너 앱 등)
